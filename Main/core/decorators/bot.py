...
def on_command(
    ...
    def decorator(func):
        async def wrapper(client: Client, message: Message):
            ...
            if sudo_only and not message.from_user.id if message.from_user in Config.SUDO_USERS else True:
                return await message.reply(f"Command {cmd} can only be used by sudousers.")
            if group_only and message.chat_type not in "supergroup":
                return await message.reply(f"Command {cmd} can only be used in a group chat.")
            if channel_only and message.chat_type != "channel":
                return await message.reply(f"Command {cmd} can only be used in a channel.")
            if private_only and message.chat_type != "private":
                return await message.reply(f"Command {cmd} can only be used in a private chat.")
            if requires_input and message.input == '':
                return await message.reply(f"An input is required to execute {cmd} command.")
            if requires_reply and not message.reply_to_message:
                return await message.reply(f"A reply is required to execute {cmd} command.")
            if message.args or message.kwargs:
                if requires_input_if_arguments and not message.input:
                    return await message.reply(f"An input is required if arguments are passed to {cmd} command.")
                if requires_reply_if_arguments and not message.reply_to_message:
                    return await message.reply(f"A reply is required if arguments are passed to {cmd} command.")
            if (
                ...
                return await message.reply(f"A channel can't execute {cmd} command.")
            if not multiple_args and len(message.args) > 1:
                return await message.reply(f"Command {cmd} doesn't allow multiple arguments.")
